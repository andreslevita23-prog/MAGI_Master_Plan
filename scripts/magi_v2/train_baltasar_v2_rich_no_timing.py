...
DEFAULT_OUTPUT_DIR = Path("data/output/magi_v2/baltasar_v2_rich_no_timing")
...
TIMING_FEATURES = {"hour", "weekday", "session", "regime"}
...
def main() -> int:
    ...
    feature_columns, removed_features = remove_timing_features(original_features)
    ...
    logging.info("Removed timing features: %s", removed_features)
    logging.info("Training with %s no-timing features", len(feature_columns))
    ...
    payload = {
        "schema_version": "baltasar_v2_rich_no_timing_model_v0.1",
        "trained_at": utc_now(),
    ...
    joblib.dump(payload, output_dir / "baltasar_v2_rich_no_timing_model.joblib")

    metrics = build_metrics(pipeline, train_df, validation_df, test_df, feature_columns, removed_features)
    (output_dir / "baltasar_v2_rich_no_timing_metrics.json").write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
    importances = feature_importances(pipeline, validation_df, feature_columns)
    write_feature_importance(output_dir / "baltasar_v2_rich_no_timing_feature_importance.csv", importances)
    (output_dir / "baltasar_v2_rich_no_timing_summary.md").write_text(markdown_summary(metrics, importances), encoding="utf-8")
    logging.info("Outputs written: %s", output_dir)
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Baltasar v2 rich features without direct timing variables.")
...
def remove_timing_features(features: list[str]) -> tuple[list[str], list[str]]:
    removed = [feature for feature in features if feature in TIMING_FEATURES or feature.startswith("time_")]
...
def verify_no_leakage(feature_columns: list[str], diagnostic_columns: list[str]) -> None:
    forbidden = sorted(set(feature_columns) & (FORBIDDEN_FEATURES | set(diagnostic_columns) | TIMING_FEATURES))
    if forbidden:
...
def build_metrics(
    ...
    return {
        "schema_version": "baltasar_v2_rich_no_timing_metrics_v0.1",
...
def feature_importances(pipeline: Pipeline, validation_df: pd.DataFrame, feature_columns: list[str]) -> list[dict[str, Any]]:
    sample = validation_df.sample(n=min(12000, len(validation_df)), random_state=818)
