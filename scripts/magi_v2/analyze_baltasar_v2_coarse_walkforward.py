...
DEFAULT_MODEL = Path("data/output/magi_v2/baltasar_v2_rich_coarse_timing/baltasar_v2_rich_coarse_timing_model.joblib")
DEFAULT_COARSE_METRICS = Path("data/output/magi_v2/baltasar_v2_rich_coarse_timing/baltasar_v2_rich_coarse_timing_metrics.json")
DEFAULT_OUTPUT_DIR = Path("data/output/magi_v2/baltasar_v2_rich_coarse_timing/walk_forward")
DEFAULT_DOC = Path("docs/baltasar_v2_coarse_walkforward.md")
...
def main() -> int:
    ...
    comparisons = build_comparisons(Path(args.coarse_metrics))
    ...
    metrics = {
        "schema_version": "baltasar_v2_rich_coarse_walkforward_v0.1",
    ...
    (output_dir / "coarse_walkforward_metrics.json").write_text(
        json.dumps(metrics, indent=2, sort_keys=True),
        encoding="utf-8",
    )

    summary = markdown_summary(metrics)
    (output_dir / "coarse_walkforward_summary.md").write_text(summary, encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze Baltasar v2 rich coarse timing walk-forward and segments.")
    ...
    parser.add_argument("--coarse-metrics", default=str(DEFAULT_COARSE_METRICS), help="Existing coarse model metrics JSON.")
...
def build_segment_metrics(df: pd.DataFrame, predictions: dict[str, pd.Series]) -> dict[str, list[dict[str, Any]]]:
    ...
    out = {name: grouped_metrics(work, oos_predictions, column) for name, column in segment_columns.items()}
    out["direction"] = direction_segment_metrics(work, oos_predictions)
    return out


def direction_segment_metrics(df: pd.DataFrame, predictions: dict[str, pd.Series]) -> list[dict[str, Any]]:
...
def build_comparisons(coarse_metrics_path: Path) -> dict[str, Any]:
    coarse = load_json(coarse_metrics_path)
    rich = load_json(RICH_TIMING_METRICS)
    no_timing = load_json(NO_TIMING_METRICS)
    pure = load_json(PURE_DIRECTIONAL_METRICS)
    return {
        "baltasar_v2_rich_coarse_timing": compact_thresholds(coarse.get("thresholds", {})),
        "baltasar_v2_rich_full_timing": compact_thresholds(rich.get("thresholds", {})),
...
def markdown_summary(metrics: dict[str, Any]) -> str:
    threshold = "0.40"
    yearly_040 = filter_threshold(metrics["temporal"]["yearly"], "0.40")
    quarterly_040 = filter_threshold(metrics["temporal"]["quarterly"], "0.40")
    monthly_040 = filter_threshold(metrics["temporal"]["monthly"], "0.40")
    monthly_040_oos = [row for row in monthly_040 if row["segment"] >= "2024-01"]
    yearly_050 = filter_threshold(metrics["temporal"]["yearly"], "0.50")
    quarterly_050 = filter_threshold(metrics["temporal"]["quarterly"], "0.50")
    monthly_050 = filter_threshold(metrics["temporal"]["monthly"], "0.50")
    monthly_050_oos = [row for row in monthly_050 if row["segment"] >= "2024-01"]
    session = filter_threshold(metrics["segments"]["session"], threshold)
    hour = filter_threshold(metrics["segments"]["hour_bucket"], threshold)
    weekday = filter_threshold(metrics["segments"]["weekday_bucket"], threshold)
    direction = filter_threshold(metrics["segments"]["direction"], threshold)
    comparisons = metrics["comparisons"]
    best_months = best_rows(monthly_040_oos, "avg_r", 8, reverse=True)
    worst_months = best_rows(monthly_040_oos, "avg_r", 8, reverse=False)
    ...
    lines = [
        ...
        "",
        table_from_rows(summary_rows(yearly_040)),
        "",
        "## Quarterly Stability at Threshold 0.40",
        "",
        table_from_rows(summary_rows(quarterly_040)),
        "",
        ...
        "",
        table_from_rows(summary_rows(yearly_050)),
        "",
        "## Quarterly Stability at Threshold 0.50",
        "",
        table_from_rows(summary_rows(quarterly_050)),
        "",
        ...
        "",
        table_from_rows(summary_rows(weekday)),
        "",
        "## Predicted Direction at Threshold 0.40",
        "",
        table_from_rows(summary_rows(direction)),
        "",
        "## Comparison Snapshot",
        "",
...
def interpretation(metrics: dict[str, Any]) -> str:
    test_coarse_040 = metrics["comparisons"]["baltasar_v2_rich_coarse_timing"]["test"]["0.40"]
    test_coarse_050 = metrics["comparisons"]["baltasar_v2_rich_coarse_timing"]["test"]["0.50"]
    test_rich = metrics["comparisons"]["baltasar_v2_rich_full_timing"]["test"]["0.50"]
    ...
    return (
        f"Coarse timing at threshold 0.40 has test avg R `{format_value(test_coarse_040.get('avg_r'))}` and PF "
        f"`{format_value(test_coarse_040.get('profit_factor'))}` with broad coverage. At threshold 0.50 it has avg R "
        f"`{format_value(test_coarse_050.get('avg_r'))}` and PF `{format_value(test_coarse_050.get('profit_factor'))}`. "
        f"Full timing threshold 0.50 has avg R `{format_value(test_rich.get('avg_r'))}`, versus no_timing avg R "
        f"`{format_value(test_no_timing.get('avg_r'))}` and Baltasar v1 avg R "
        f"`{format_value(test_v1.get('avg_r'))}`. Positive OOS months at threshold 0.50 with adequate sample are "
        f"`{positive_months}/{total_months}`. Coarse timing reduces exact-hour dependence, but bad quarters still require "
        "blocking or a meta-policy before this becomes the main Baltasar v2 candidate."
    )
...
def comparison_snapshot(comparisons: dict[str, Any]) -> str:
    rows = []
    for name, data in [
        ("coarse_timing", comparisons["baltasar_v2_rich_coarse_timing"]),
        ("rich_full_timing", comparisons["baltasar_v2_rich_full_timing"]),
